...
class MoondreamConfig:
    ...
    def from_dict(cls, config_dict: dict) -> "MoondreamConfig":
        ...
        def filter_fields(dc_class, d: dict) -> dict:
        ...
        if "moe" in text_dict and text_dict["moe"] is not None:
            moe_dict = filter_fields(TextMoeConfig, text_dict["moe"])
            text_dict = {**text_dict, "moe": TextMoeConfig(**moe_dict)}
        text_dict = filter_fields(TextConfig, text_dict)

        text_config = TextConfig(**text_dict) if text_dict else TextConfig()
        vision_config = VisionConfig(**filter_fields(VisionConfig, config_dict.get("vision", {})))
        region_config = RegionConfig(**filter_fields(RegionConfig, config_dict.get("region", {})))
        tokenizer_config = TokenizerConfig(**filter_fields(TokenizerConfig, config_dict.get("tokenizer", {})))
