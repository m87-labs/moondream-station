...
def test_model_capabilities(child, model_name, image_url):
    """
...
def test_all_models(child, manifest_url, image_url):
    ...
    for model_name in models:
        ...
        try:
            ...
            test_model_capabilities(child, model_name, image_url)
...
def test_server(cleanup=False, executable_path='./moondream_station', server_args=None, 
                manifest_url=None, skip_validation=False, image_url=DEFAULT_IMAGE_URL):
    if cleanup:
        clean_files()
    ...
    with server_session(executable_path, server_args) as child:
        ...
        test_all_models(child, validation_url, image_url)
...
def main():
    ...
    parser.add_argument('--image', default=DEFAULT_IMAGE_URL,
                        help='Image URL to use for testing (default: Moondream logo). Please provide a compatible expected_responses.json file with this image URL.')
